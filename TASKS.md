...
- [x] 3. [MEDIUM] Trim whitespace from search query in `filterEvents()` and `getActiveFilterCount()` to prevent whitespace-only queries from showing zero results
- [x] 4. [MEDIUM] Add a proper confirmation i18n key (`eventFilters.filtersSaved`) for the save snackbar instead of reusing the section title `eventFilters.saveFilter`
- [x] 5. [MEDIUM] Add try/catch error handling around `persistFilters()` call in the save action of `EventFiltersPage`
- [x] 6. [MEDIUM] Trim search query in `EventFilterCubit.updateSearchQuery()` or in `filterEvents()` to handle whitespace-padded input (can be combined with task 3)
