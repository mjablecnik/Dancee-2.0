...
- [x] 6. [MEDIUM] Add `findCourseByOriginalUrl` duplicate check in `batch.ts` (`processAll` and `processSingle`) before scheduling workflows
- [x] 7. [MEDIUM] Document `_contains` JSON substring matching limitation in `findExpiredEventWithImage` or store primary dance as a separate indexed field
- [x] 8. [MEDIUM] Add property-based test (fast-check) for translation info_translations length preservation (Design Property 3)
- [x] 9. [MEDIUM] Add property-based test (fast-check) for parent dance style filtering includes child codes (Design Property 8)
