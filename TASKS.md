...
- [ ] 2. [HIGH] Add `listDanceStyles` handler to `backend/dancee_workflow/src/services/api.ts` that returns the full dance styles list via the API
...
- [ ] 7. [HIGH] Modify `extractDanceTypes()` in `lib/features/events/logic/event_filter.dart` to accept a `List<DanceStyle>` parameter and return only parent style codes, aggregating child style event counts into their parent
