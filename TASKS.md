...
- [x] 9. [MEDIUM] Add Event.fromJson() factory or document that toJson() is not round-trippable with fromDirectus()
- [x] 10. [MEDIUM] Log warning in EventPart.fromDirectus when start time is missing instead of silently using DateTime.now()
- [x] 11. [MEDIUM] Add dateFrom/dateTo validation in EventFiltersPage._updateDraft to prevent invalid ranges
- [x] 12. [MEDIUM] Add test for FilterPersistenceService behavior when SharedPreferences write fails
- [x] 13. [MEDIUM] Add safeguard in EventRepository.getFavoriteEvents for large favorite ID lists exceeding URL limits
- [x] 14. [MEDIUM] Add test for EventListCubit error recovery after toggleFavorite failure
