...
- [-] 1. Wire navigation from event list and favorites to event detail page
  - [-] 1.1 Update `EventsByDateSection` in `lib/features/events/pages/event_list/sections.dart` to navigate on event card tap
  ...
  - [~] 1.2 Update `FavoritesPage` in `lib/features/events/pages/favorites_page.dart` to navigate on event card tap
    - Replace the `// TODO: Navigate to event detail` placeholders in both upcoming and past event lists
    - Import `event_detail_page.dart` and use `EventDetailRoute(id: event.id).go(context)` for type-safe navigation
    - _Requirements: 1.1_

- [ ] 2. Add `url_launcher` dependency and create `EventDetailCubit`
  - [~] 2.1 Add `url_launcher` package to `pubspec.yaml` and run `task get-deps`
    - _Requirements: 4.3, 7.3, 10.1_

  - [~] 2.2 Create `lib/features/events/logic/event_detail.dart` with `EventDetailCubit` and `EventDetailState` (freezed)
  ...
  - [~] 2.3 Write property test: favorite toggle round trip (Property 6)
  ...
  - [~] 2.4 Write property test: favorite toggle error recovery (Property 7)
  ...
  - [~] 2.5 Write property test: map URL construction (Property 5)
...
- [ ] 3. Register `EventDetailCubit` in DI and add translation keys
  - [~] 3.1 Register `EventDetailCubit` as `factoryParam` in `lib/core/service_locator.dart`
    - Use `registerFactoryParam<EventDetailCubit, String, void>` with eventId parameter
    - Import the new cubit file
    - _Requirements: 9.1_

  - [~] 3.2 Add new translation keys to all three language files (`strings.i18n.json`, `strings_cs.i18n.json`, `strings_es.i18n.json`)
...
- [~] 4. Checkpoint - Ensure cubit, DI, and navigation compile correctly
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Wire `EventDetailCubit` into the page and update sections
  - [~] 5.1 Update `EventDetailPage` to use `BlocProvider<EventDetailCubit>` instead of directly reading `EventListCubit`
  ...
  - [~] 5.2 Update `EventDetailHeaderSection` to accept and use favorite/map callbacks
  ...
  - [~] 5.3 Update `EventVenueSection` to accept and use navigate callback
    - Accept `onNavigatePressed` callback
    - Wire the navigate button's `onPressed` to the callback
    - _Requirements: 4.2, 4.3_

  - [~] 5.4 Update `EventInfoSection` and `EventInfoCard` to handle URL taps
  ...
  - [~] 5.5 Write property test: favorite icon reflects event state (Property 1)
  ...
  - [~] 5.6 Write property test: time range formatting (Property 4)
  ...
  - [~] 5.7 Write property test: date formatting (Property 3)
  ...
  - [~] 5.8 Write property test: event part type label mapping (Property 8)
