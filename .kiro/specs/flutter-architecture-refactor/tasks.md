...
- [x] 1. Phase 1: Setup and Infrastructure
  - [x] 1.1 Add go_router, freezed, and build_runner dependencies to pubspec.yaml
  ...
  - [x] 1.2 Create new directory structure scaffolding
  ...
  - [x] 1.3 Configure build_runner for code generation
    - Create or update `build.yaml` for go_router_builder and freezed configuration
    - Ensure generated files are in `.gitignore`
    - _Requirements: 15.1, 15.2, 15.3, 15.6_

- [x] 2. Checkpoint - Ensure dependencies install and build_runner config is valid
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Phase 2: Events Feature - Data Layer
  - [x] 3.1 Create event entities with fromJson/toJson in `lib/features/events/data/entities.dart`
  ...
  - [x] 3.2 Write property test for entity serialization round-trip
    - **Property 1: Entity Serialization Round-Trip**
    - **Validates: Requirements 2.2, 6.2, 6.3**

  - [x] 3.3 Write property test for entity value equality
    - **Property 3: Entity Value Equality**
    - **Validates: Requirements 6.5**

  - [x] 3.4 Migrate EventRepository to `lib/features/events/data/event_repository.dart`
  ...
  - [x] 3.5 Write property test for repository error handling
    - **Property 4: Repository Error Handling**
    - **Validates: Requirements 7.5**

  - [x] 3.6 Write property test for repository data validation
    - **Property 5: Repository Data Validation**
    - **Validates: Requirements 7.6**

  - [x] 3.7 Write unit tests for EventRepository
...
- [x] 4. Phase 2: Events Feature - Logic Layer
  - [x] 4.1 Create EventListCubit with freezed state in `lib/features/events/logic/event_list.dart`
  ...
  - [x] 4.2 Create FavoritesCubit with freezed state in `lib/features/events/logic/favorites.dart`
    - Combine cubit + state in one file using freezed
  ...
  - [x] 4.3 Write property test for cubit exception handling
    - **Property 7: Cubit Exception Handling**
    - **Validates: Requirements 11.4, 11.5**

  - [x] 4.4 Write unit tests for EventListCubit and FavoritesCubit
...
- [x] 5. Phase 2: Events Feature - Presentation Layer
  - [x] 5.1 Create EventListPage with route definition in `lib/features/events/pages/event_list/`
  ...
  - [x] 5.2 Extract sections for EventListPage
  ...
  - [x] 5.3 Extract components for EventListPage
  ...
  - [x] 5.4 Create EventDetailPage with route definition in `lib/features/events/pages/event_detail/`
  ...
  - [x] 5.5 Create EventFiltersPage in `lib/features/events/pages/event_filters_page.dart`
  ...
  - [x] 5.6 Create FavoritesPage in `lib/features/events/pages/favorites_page.dart`
...
- [x] 6. Checkpoint - Events feature compiles and functions correctly
  - Run `task build-runner` to generate routes and freezed classes
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Phase 3: App Feature
  - [x] 7.1 Create MainLayout with shell route in `lib/features/app/layouts.dart`
  ...
  - [x] 7.2 Create app pages (InitialPage, NotFoundPage, ErrorPage)
  ...
  - [x] 7.3 Create GoRouter configuration in `lib/core/routing.dart`
  ...
  - [x] 7.4 Update main.dart to use MaterialApp.router
    - Replace `MaterialApp` with `MaterialApp.router` using `goRouter`
    - Maintain translation provider and locale settings
    - _Requirements: 10.6_

  - [x] 7.5 Write property test for authentication redirect
    - **Property 6: Authentication Redirect**
    - **Validates: Requirements 10.8**

- [x] 8. Checkpoint - App feature and routing work correctly
  - Run `task build-runner` to regenerate routes
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Phase 4: Auth Feature
  - [x] 9.1 Create auth data layer placeholders
  ...
  - [x] 9.2 Create AuthCubit with freezed state in `lib/features/auth/logic/auth.dart`
    - Placeholder cubit with states: `initial`, `loading`, `authenticated`, `unauthenticated`, `error`
    - _Requirements: 4.1_

  - [x] 9.3 Create LoginPage in `lib/features/auth/pages/login/`
  ...
  - [x] 9.4 Create RegisterPage in `lib/features/auth/pages/register/`
...
- [x] 10. Phase 5: Settings Feature
  - [x] 10.1 Create settings data layer placeholders
  ...
  - [x] 10.2 Create SettingsCubit with freezed state in `lib/features/settings/logic/settings.dart`
    - Placeholder cubit with states: `initial`, `loading`, `loaded`, `error`
    - _Requirements: 5.1_

  - [x] 10.3 Create SettingsPage in `lib/features/settings/pages/settings_page.dart`
...
- [x] 11. Checkpoint - Auth and Settings features compile with routes
  - Run `task build-runner` to regenerate all routes
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. Phase 6: Design System
  - [x] 12.1 Create color constants in `lib/design/colors.dart`
  ...
  - [x] 12.2 Create typography definitions in `lib/design/typography.dart`
    - Extract text styles into `AppTypography` class
    - Define display, body, and label text styles
    - _Requirements: 9.1, 9.5, 9.9_

  - [x] 12.3 Create theme configuration in `lib/design/theme.dart`
  ...
  - [x] 12.4 Create shared widgets in `lib/design/widgets.dart`
...
- [x] 13. Phase 7: Cleanup and Wiring
  - [x] 13.1 Consolidate core module files
  ...
  - [x] 13.2 Update dependency injection in `lib/core/service_locator.dart`
  ...
  - [x] 13.3 Update configuration management
  ...
  - [x] 13.4 Remove legacy directories
  ...
  - [x] 13.5 Verify file naming conventions across the codebase
  ...
  - [x] 13.6 Verify translation integration
...
- [x] 14. Checkpoint - Full migration complete, all legacy code removed
  - Run `task build-runner` to regenerate all code
  - Ensure all tests pass, ask the user if questions arise.

- [x] 15. Testing and Documentation
  - [x] 15.1 Create test directory structure mirroring new architecture
  ...
  - [x] 15.2 Write property test for repository return type consistency
    - **Property 2: Repository Return Type Consistency**
    - **Validates: Requirements 2.7, 6.4**

  - [x] 15.3 Update README.md with new architecture documentation
  ...
  - [x] 15.4 Update taskfile.yaml with code generation commands
